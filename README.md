...
### 📝 収集なし記録機能（業務効率化）
- **収集なしボタン**：収集物がなかった場合をワンクリックで記録
  - 視覚的にわかりやすいボタンデザイン（押すと凹む）
  - ボタン1つで通常状態と収集なしを切り替え
  - 収集一覧・収集確認タブに「収集なし」件数を表示
- **収集確認タブでフィルター**：「収集なしのみ表示」チェックボックスで絞り込み可能
  - 号車別とコース別の組み合わせフィルター対応
  - 収集なし記録の確認が簡単に

### 🚚 コース自動判別機能（完全対応版）
- **月木コース・火金コース・その他コースを自動判別**
  - 可燃ごみの収集曜日から自動的にコースを判定
  - 月曜または木曜 → 「月木コース」（青バッジ）
  - 火曜または金曜 → 「火金コース」（ピンクバッジ）
  - それ以外 → 「その他」（灰色バッジ）
- **複数データ形式に完全対応**
  - 英語曜日名キー対応（monday, friday等）
  - 数値キー対応（1, 2, 4, 5）
  - 文字列キー対応（"1", "2", "4", "5"）
  - 日本語キー対応（月曜、火曜等）
  - 配列・オブジェクト両形式に対応
- **コースバッジ表示**：収集一覧・収集確認タブで各依頼人のコースを色分け表示
- **コースフィルター**：収集一覧・地図表示・収集確認タブでコース別に絞り込み可能
- **既存データも自動判別**：現在登録されている全ての依頼に対して自動的にコースを判別

### 📸 OCR精度向上
- **Tesseract.js v5の最新設定を適用**
  - LSTM OCRエンジン使用（最高精度）
  - 自動向き検出（PSM.AUTO_OSD）
  - ノイズリダクション強化
  - 辞書ペナルティ軽減で固有名詞認識向上
...
- **収集なしボタン**：収集物がなかった場合を記録 ✨NEW! v1.8.0
  - 視覚的フィードバック（押すと凹む）
  - 統計に自動反映
- **コース自動判別・表示** ✨NEW! v1.8.0
  - 月木コース・火金コース・その他を自動判別
  - カラーバッジで視覚的に表示
  - コース別フィルター機能
- **一時中断機能（中断中の依頼を薄く表示、再開ボタン付き）**
- **廃止機能（OCRで廃止検出 → 削除確認）**
- **一時中断の表示/非表示切り替え**
- 新規登録、編集、削除機能
- **CSV一括登録機能（複数の依頼人を一度に登録）**
- **可燃収集曜日の詳細設定（毎週・第1〜5週指定）**
- **不燃収集の有無選択と曜日設定**
- 検索機能(名前・住所で検索)
- フィルター機能(号車別、ステータス別、コース別 ✨NEW!)

### 📸 写真からの自動読み取り(OCR) ✨精度向上
- 紙に書かれた収集依頼情報を撮影
- **Tesseract.js v5 最新エンジン使用** ✨NEW! v1.8.0
  - LSTM OCRエンジンで最高精度を実現
  - 自動向き検出とノイズリダクション強化
- **撮影ガイド表示で最適な撮影方法を案内**
- **フィールドごとの認識精度表示（名前・住所・日付）**
- **一時中断・廃止・再開の自動検出**
  - 紙に「一時中断」と書かれていれば自動で検出
  - 「廃止」検出時は削除確認ダイアログ表示
  - 「再開」検出時はステータスを未収集に戻す
- **住所から依頼人を自動検索してステータス変更**
- **にこやか収集フォーマットに最適化**
...
- **過去日付の履歴表示対応**
  - 過去の収集実績を確認可能
  - 日別の収集ステータスを自動記録（30日間保持）
- **コースフィルター** ✨NEW! v1.8.0
  - 月木コース・火金コース・その他コースで絞り込み
  - 号車別と組み合わせて使用可能
- **収集なしフィルター** ✨NEW! v1.8.0
  - 「収集なしのみ表示」チェックボックスで絞り込み
  - 号車別・コース別と組み合わせて使用可能
- **可燃・不燃の件数を個別表示**
- 統計表示（対象件数、未収集、収集済み、収集なし ✨NEW!）
- **一括完了ボタン（対象の未収集をまとめて収集済みに）**
- ステータスワンクリック変更にも対応

### 🔄 自動リセット機能 ✨改善！v1.8.0
- **毎日0時に自動リセット**
  - 「収集済み」と「収集なし」を「未収集」に自動変更 ✨NEW!
  - 一時中断は自動リセット対象外
  - ページ読み込み時に日付チェック
  - リセット件数を通知表示
- **変更前のステータスを履歴に自動保存**
...
### ⚙️ 設定タブ ✨NEW!
- **地域設定、ゴミ種類、カラーテーマ、バックアップを1つのタブに統合**
- タブ切り替えでスムーズな設定変更
- UIがシンプルで使いやすく
...
#### 💾 バックアップと復元 ✨NEW!
- **自動バックアップ（毎日23:00、3世代保持）**
- **手動バックアップ（JSONファイルダウンロード）**
- **データ復元機能（バックアップファイルから復元）**
- 最終バックアップ日時の表示
- 全データ（収集依頼、号車、地域、ゴミ種類、履歴）をバックアップ

### 🗺️ 地図表示機能
- OpenStreetMap + Leaflet.jsによる地図表示
- **地図表示切り替え**：通常地図 ⇔ 航空写真 ✨v1.7.0
- **ピン位置の手動編集**：マーカーをドラッグして位置調整 ✨v1.7.0
- 収集場所をピンで視覚的に表示
- 号車ごとに色分けされたマーカー
- **一時中断の依頼人は灰色のピンで表示**
- ステータス別の境界線表示
  ...
  - 灰色: 一時中断
- ピンクリックで詳細情報表示
- **コース別フィルター（月木/火金/その他）** ✨NEW! v1.8.0
- 号車別・ステータス別フィルター（一時中断含む）
- 現在地表示機能(GPS対応)
- スマホでの地図操作に最適化
- 完全無料・APIキー不要

### 📊 統計情報
- 総件数、未収集件数、収集済み件数、**収集なし件数** ✨NEW! v1.8.0、一時中断件数の表示
- リアルタイムで更新
...
| status | text | 収集ステータス(未収集/収集済み/一時中断) |
| manual_assignment | bool | 手動割り当てフラグ |
| notes | text | 備考 |
| **latitude** | **float** | **緯度（手動設定した場合）** ✨NEW! |
| **longitude** | **float** | **経度（手動設定した場合）** ✨NEW! |
...
### v1.9.0 (2026-02-28) ✨最新 - シンプル化・効率化アップデート
- 🚀 **ステータス機能の簡素化**
  - 「未収集」「収集済み」ステータスを廃止
  - 「一時中断」と「収集なし」のみで管理
  - 編集モーダルからステータス選択を削除
...
- 🚀 **収集なし記録機能（完全版）**
  - 視覚的フィードバック（ボタンが凹む）
  - 収集確認タブに「収集なしのみ表示」チェックボックス追加
  - 号車別・コース別との組み合わせフィルター対応
- 🚀 **コース自動判別機能（完全対応版）** ✨完成!
  - 英語曜日名キー対応（monday, tuesday, thursday, friday）
  - 数値キー対応（1, 2, 4, 5）
  - 文字列キー対応（"1", "2", "4", "5"）
  - 日本語キー対応（月曜、火曜、木曜、金曜）
  - 配列・オブジェクト両形式に対応
  - enabled: true チェック対応
  - 収集一覧・収集確認タブにコースバッジ表示
  - 収集一覧・地図表示・収集確認タブでコース別フィルター
  - 既存データも自動判別（全データに対応）
- ✨ 統計表示の最適化
  - 総件数、収集なし、一時中断のみ表示
  - 未収集・収集済みカウントを削除
...
### v1.8.0 (2026-02-27) - 業務効率化アップデート
- 🚀 **収集なし記録機能** - 収集物がなかった場合をワンクリックで記録
  - 視覚的にわかりやすいボタン（押すと凹む）
  - ステータスが「収集なし」に変更され、統計に自動反映
  - 収集確認タブに「収集なしのみ表示」フィルター追加
- 🚀 **コース自動判別機能** - 可燃ごみの曜日から自動的にコースを判定
  - 月木コース・火金コース・その他コースの3種類
  - 収集一覧タブにカラーバッジで表示
  - 収集一覧・地図表示・収集確認タブでコース別フィルター機能追加 ✨NEW!
  - 既存データも自動判別（登録済みの30件にも対応）
- 🚀 **自動リセット機能の改善** ✨NEW!
  - 毎日0時に「収集済み」と「収集なし」を「未収集」に自動リセット
  - 一時中断は自動リセット対象外
  - ページ読み込み時に日付をチェックし自動実行
  - リセット件数を通知表示
- 🚀 **OCR精度向上** - Tesseract.js v5の最新エンジン設定
  - LSTM OCRエンジンで最高精度を実現
  - 自動向き検出（PSM.AUTO_OSD）
  - ノイズリダクション強化
  - 辞書ペナルティ軽減で固有名詞認識向上
...
### v1.7.0 (2026-02-25) - 地図機能強化アップデート
- 🚀 **地図表示切り替え** - 通常地図 ⇔ 航空写真をワンクリックで切り替え
- 🚀 **ピン位置の手動編集** - 地図上で正確な位置を設定可能
  - 住所から自動取得した位置が不正確な場合でも安心
  - ポップアップから「ピン修正」ボタンでマーカーをドラッグ
  - 緯度・経度を自動保存し、次回から正確な位置を表示
- ✨ 編集モーダルを簡略化（地図編集は地図タブから）
- ✨ collectionsテーブルにlatitude/longitudeカラム追加
- 🔧 地図表示の安定性向上

### v1.6.0 (2026-02-23) - 大規模アップデート
- 🚀 **収集履歴記録機能** - ステータス変更を日別に自動記録（30日間保持）
- 🚀 **自動リセット機能** - 毎日0時に収集済み→未収集へ自動変更
- 🚀 **収集確認機能の大幅改善** - 曜日・週数ベースのスマート検索
  - 選択した日付の曜日と週番号で自動判定
  - 可燃・不燃の収集スケジュールに対応
  - 号車の稼働日も考慮
  - 過去日付の履歴表示対応
  - 可燃・不燃の件数を個別表示
- 🚀 **一時中断・廃止・再開機能**
  - 一時中断ステータスの追加（中断中は薄く表示）
  - OCRで「一時中断」「廃止」「再開」を自動検出
  - 住所から依頼人を自動検索してステータス変更
  - 再開ボタンで未収集に復帰
  - 一時中断の表示/非表示切り替え
  - **地図上で一時中断のピンを灰色表示** ✨NEW!
- 🚀 **設定タブの統合** - 地域設定、ゴミ種類、カラーテーマ、バックアップを1つのタブに集約
- 🚀 **バックアップ機能**
  - 自動バックアップ（毎日23:00、3世代保持）
  - 手動バックアップ（JSONファイルダウンロード）
  - データ復元機能（バックアップファイルから復元）
- ✨ タブ構成の最適化（9タブ→7タブへ簡素化）
- ✨ 一時中断の統計カウント追加
- ✨ 地図の凡例に一時中断を追加
- 🔧 UI/UXの全面改善

### v1.5.0 (2026-02-21)
- 🚀 **カラーテーマ変更機能**（6種類のテーマから選択可能）
- 🚀 **収集確認タブ**（日付別・号車別で未収集を確認）
- 🚀 **CSV一括登録機能**（複数の依頼人を一度に登録）
- 🚀 **OCR機能改善**（撮影ガイド、精度表示）
- ✨ ステータスワンクリック変更機能
- ✨ UIの最適化（レスポンシブデザイン改善）
...
### v1.4.0 (2026-02-14)
- 🚀 **Google Maps Geocoding APIへ完全移行**
- ✨ 認識率が0%（フォールバック）→ ほぼ100%に大幅改善
- ✨ 表示位置が西宮市中心部（推定）→ 正確な番地レベルに向上
- ✨ 住所の完全パース対応（番地レベルまで認識）
- 💰 月40,000リクエストまで完全無料（月$200の無料クレジット）
- 💰 1リクエスト = $0.005（約0.5円）
- 💰 月10,000件登録しても$0（無料枠内）
...
### v1.3.0 (2026-02-13)
- 🚀 **Supabase完全移行**（PostgreSQL + GitHub Pages）
- ✨ 完全無料で永続的に使用可能
- ✨ 高速・安定なデータベース
- 🔧 API構造の最適化

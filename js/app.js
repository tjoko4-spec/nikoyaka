...
// 可燃ゴミ曜日からコースを判別
function detectCourse(combustibleDays) {
    if (!combustibleDays) {
        return 'その他';
    }
    ...
    try {
        // JSON文字列の場合はパース
        const parsedData = typeof combustibleDays === 'string' ? JSON.parse(combustibleDays) : combustibleDays;
        
        // データ構造を確認
        const daysData = parsedData.days || parsedData || {};
        
        // 🔍 デバッグ: 最初の1回だけ詳細ログ出力
        if (!window.courseDebugShown) {
            console.log('━━━━ コース判別デバッグ ━━━━');
            console.log('元データ:', combustibleDays);
            console.log('パース後:', parsedData);
            console.log('days:', daysData);
            console.log('利用可能なキー:', Object.keys(daysData));
            window.courseDebugShown = true;
        }
        
        // 各曜日にデータがあるかチェック
        // 数値キー、文字列キー、英語名キーすべてをチェック
        const hasDay = (dayNum, dayNameEn, dayNameJp) => {
            const keys = [
                String(dayNum),      // "1", "2", "4", "5"
                dayNum,              // 1, 2, 4, 5
                dayNameEn,           // "monday", "tuesday", "thursday", "friday"
                dayNameJp            // "月曜", "火曜", "木曜", "金曜"
            ];
            
            for (const key of keys) {
                let dayData = daysData[key];
                if (!dayData) continue;
                ...
                // enabled: true をチェック
                if (dayData && dayData.enabled === true) {
                    return true;
                }
                
                // weeks配列が存在し、長さが0より大きい
                if (dayData && dayData.weeks && Array.isArray(dayData.weeks) && dayData.weeks.length > 0) {
                    return true;
                }
            }
        ...
        const hasMonday = hasDay(1, 'monday', '月曜');
        const hasTuesday = hasDay(2, 'tuesday', '火曜');
        const hasThursday = hasDay(4, 'thursday', '木曜');
        const hasFriday = hasDay(5, 'friday', '金曜');
        
        // 🔍 デバッグ: 曜日チェック結果
        if (!window.courseDebugShown2) {
            console.log('曜日チェック結果:', { hasMonday, hasTuesday, hasThursday, hasFriday });
            console.log('━━━━━━━━━━━━━━━━━━━━');
            window.courseDebugShown2 = true;
        }
        
        // 月木コース: 月曜または木曜がある（かつ火金ではない）
        if ((hasMonday || hasThursday) && !hasTuesday && !hasFriday) {
            return '月木コース';
        }
        ...
        // 月木の曜日がある場合
        if (hasMonday || hasThursday) {
            return '月木コース';
        }
        
        return 'その他';
    } catch (error) {
        console.error('❌ コース判別エラー:', error);
        return 'その他';
    }
}
...
function updateStats() {
    ...
    const noCollectionCount = collections.filter(c => c.status === '収集なし').length;
    const pausedCount = collections.filter(c => c.status === '一時中断').length;
    
    document.getElementById('totalCount').textContent = totalCount;
    document.getElementById('noCollectionCount').textContent = noCollectionCount;
...
function renderCollections() {
    ...
    if (collections.length > 0) {
        ...
        console.log('━━━━━━━━━━━━━━━━━━━━━━');
        console.log('📋 最初のデータ:', firstItem.name);
        console.log('📋 combustible_days (元データ):', firstItem.combustible_days);
        console.log('📋 データ型:', typeof firstItem.combustible_days);
        
        if (firstItem.combustible_days) {
            try {
                const parsed = typeof firstItem.combustible_days === 'string' 
                    ? JSON.parse(firstItem.combustible_days) 
                    : firstItem.combustible_days;
                console.log('📋 パース後:', parsed);
                console.log('📋 parsed.days:', parsed.days);
                if (parsed.days) {
                    console.log('📋 利用可能な曜日:', Object.keys(parsed.days));
                    Object.keys(parsed.days).forEach(dayKey => {
                        console.log(`   ${dayKey}:`, parsed.days[dayKey]);
                    });
                }
            } catch (e) {
    ...
    let filtered = collections.filter(c => {
        ...
        // コースフィルター
        if (filterCourse) {
            const course = detectCourse(c.combustible_days);
    ...
    listElement.innerHTML = filtered.map(item => {
        const course = detectCourse(item.combustible_days);
        const courseBadgeClass = course === '月木コース' ? 'course-badge-mon-thu' : 
                                 course === '火金コース' ? 'course-badge-tue-fri' : 
                                 'course-badge-other';
        ...
        <div class="collection-card status-${item.status}">
            <div class="collection-header">
                <div class="collection-name">
                    ${item.name || '未設定'}
                    <span class="course-badge ${courseBadgeClass}">${course}</span>
                </div>
                <button class="collection-status-btn ${item.status === '収集なし' ? 'active' : ''}" 
                        onclick="toggleNoCollection('${item.id}', '${item.status}')" 
                        title="収集物がなかった場合にクリック">
                    <i class="fas ${item.status === '収集なし' ? 'fa-check-circle' : 'fa-times-circle'}"></i>
                    収集なし
                </button>
            </div>
...
// 収集なしボタンをクリック
async function toggleNoCollection(collectionId, currentStatus) {
    // 収集なし ⇔ 通常（statusなし） を切り替え
    const nextStatus = currentStatus === '収集なし' ? null : '収集なし';
    
    try {
        // Supabaseに保存
        await SupabaseAPI.updateCollection(collectionId, { status: nextStatus });
        ...
        const message = nextStatus === '収集なし' ? '収集物がなかったことを記録しました' : '収集なしを解除しました';
...
async function togglePauseStatus(collectionId, currentStatus) {
    ...
    const nextStatus = currentStatus === '一時中断' ? null : '一時中断';
    ...
    try {
        ...
        const statusLabel = nextStatus === '一時中断' ? '一時中断' : '通常';
...
async function handleEditForm(e) {
    ...
    console.log('📊 編集データ:', { name, address, startDate, vehicleId });
...
async function processOCR() {
    ...
    try {
        ...
        // Tesseract.js v5のパラメータ設定（精度最大化）
        await worker.setParameters({
            tessedit_pageseg_mode: Tesseract.PSM.AUTO_OSD, // 自動向き検出とレイアウト解析
            preserve_interword_spaces: '1', // 単語間スペース保持
            tessedit_char_whitelist: '', // ホワイトリストなし（全文字許可）
            // 日本語認識を優先
            language_model_penalty_non_dict_word: '0.3', // 辞書にない単語のペナルティを軽減
            language_model_penalty_non_freq_dict_word: '0.3', // 頻度の低い単語のペナルティを軽減
            // v5の追加設定
            tessedit_ocr_engine_mode: '2', // LSTM OCRエンジン（最高精度）
            textord_heavy_nr: '1', // ノイズリダクション強化
        });
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('filterCourse').addEventListener('change', renderCollections);
    ...
    document.getElementById('mapFilterCourse').addEventListener('change', updateMapMarkers);
...
function initConfirmationView() {
    ...
    // コースフィルター
    document.getElementById('confirmCourse').addEventListener('change', searchConfirmations);
    
    // 収集なしのみ表示チェックボックス
    document.getElementById('showNoCollectionOnly').addEventListener('change', searchConfirmations);
}
...
function searchConfirmations() {
    ...
    const courseFilter = document.getElementById('confirmCourse').value;
    const showNoCollectionOnly = document.getElementById('showNoCollectionOnly').checked;
    ...
    let filtered = collections.filter(c => {
        ...
        if (courseFilter) {
            const course = detectCourse(c.combustible_days);
            if (course !== courseFilter) return false;
        }
        
        // 収集なしのみ表示フィルタ
        if (showNoCollectionOnly) {
            // 収集なしのみを表示する場合は、ステータスが「収集なし」のものだけ
            return c.status === '収集なし';
        }
        
        // 収集なしステータスは常に表示（日付に関係なく）
        if (c.status === '収集なし') {
            return true;
        }
        
        // 曜日・週番号でフィルタ（可燃ごみのスケジュールをチェック）
        if (c.combustible_days) {
            try {
                const parsedData = typeof c.combustible_days === 'string' 
                    ? JSON.parse(c.combustible_days) 
                    : c.combustible_days;
                ...
                for (const key of dayKeys) {
                    ...
                    // enabled: true またはweeks配列をチェック
                    if (dayData.enabled === true) {
                        // weeks配列があれば、今週が含まれているかチェック
                        if (dayData.weeks && Array.isArray(dayData.weeks)) {
                            const weeksIncluded = dayData.weeks.includes(weekOfMonth) || 
                                                 dayData.weeks.includes('every') || 
                                                 dayData.weeks.includes('毎週');
                            if (weeksIncluded) return true;
                        } else {
                            // weeks指定なしの場合は該当とみなす
                            return true;
                        }
                    } else if (dayData.weeks && Array.isArray(dayData.weeks) && dayData.weeks.length > 0) {
                        // enabled指定なし、weeks配列がある場合
                        const weeksIncluded = dayData.weeks.includes(weekOfMonth) || 
                                             dayData.weeks.includes('every') || 
                                             dayData.weeks.includes('毎週');
                        if (weeksIncluded) return true;
                ...
                console.error('収集スケジュール解析エラー:', error);
            }
        }
...
function renderConfirmationList(items) {
    ...
    const noCollection = items.filter(i => i.status === '収集なし').length;
    const paused = items.filter(i => i.status === '一時中断').length;
    
    document.getElementById('confirmTotal').textContent = total;
    document.getElementById('confirmNoCollection').textContent = noCollection;
    document.getElementById('confirmPaused').textContent = paused;
...
function renderThemesSettings() {
    ...
    const themeDescriptions = {
        ...
        pink: '柔らかく親しみやすい',
        gray: 'シンプルで洗練された',
        teal: '目の疲れを軽減、長時間作業向け',
        amber: '屋外や明るい場所で見やすい',
        navy: 'はっきりした高コントラスト',
        forest: '自然な緑でリラックス効果',
        crimson: '注意喚起、緊急時にも見やすい',
        indigo: '上品で個性的な深い紫'
    };
    ...
    grid.innerHTML = Object.keys(themes).map(key => {
        ...
        <div class="theme-card" data-theme="${key}">
            <div class="theme-preview">
                <div class="theme-header" style="background: ${theme.headerGradient};"></div>
                <div class="theme-body">
                    <div class="theme-sample theme-sample-primary" style="background: ${theme.primary};"></div>
                    <div class="theme-sample theme-sample-success" style="background: ${theme.primary};"></div>
                    <div class="theme-sample theme-sample-bg" style="background: ${theme.background};"></div>
                </div>
            </div>
            <div class="theme-info">
                <h3 class="theme-name">${theme.name}</h3>
                <p class="theme-description">${themeDescriptions[key] || ''}</p>
            </div>
            <button class="btn btn-primary theme-select-btn" onclick="applyTheme('${key}')">
    ...
    // 現在のテーマをアクティブ表示
    const currentTheme = localStorage.getItem('nikoyaka-theme') || 'green';
    const activeCard = grid.querySelector(`.theme-card[data-theme="${currentTheme}"]`);
    if (activeCard) {
        activeCard.classList.add('active');
    }
}
...
async function updateMapMarkers() {
    ...
    const filterCourse = document.getElementById('mapFilterCourse').value;
